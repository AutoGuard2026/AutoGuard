#app_type:1media 2messaging 3navigation 4POI 5IOT （6video 7games 8browsers） 6weather【待改！！！之前缺了messaging】
...
def get_instruction(i,j):
    #j代表返回值的类型：1是instruction-rules1;2是process;3是rules2
    rules1 = [0]  # 记录待检测的点击链规则
    rules2 = [0]  # 记录待检测的全局性规则
    process = [0]  # 与rules1对应，提供预设的检测思路
    match app_type:
        case 1:  # Media
            app_type_s="media"
            rules1[0] = 1
            # rules1.append("EP-2 When the app is relaunched from the home screen, the app must restore the app state as closely as possible to the previous state.")
            # process.append("tap the app icon, click something to change the state, relaunch(home and open the app), and compare states before and after relaunch")
        ...
        case 3:  # Navigation
            ...
            process.append("try to discover more different page in this app and don't open other app")
            # rules1.append("NF-6 The app must handle navigation requests from other Android Auto apps.")
            # process.append("launch messaging app and process the navigation requests from the first contact")
    ...
    if j==1:
        # if app_type==1:
        #     instruction="try to discover more different page or different functions"
        # else:
...
def get_action_prompt(i,clickable_infos, width, height,keyboard, summary_history, action_history,add_info, memory,error_flag=False, last_action="",free_flag=False):
    print(f"free_flag is: {free_flag}")  # 调试输出
    prompt=''
    ...
    for num in range(len(clickable_infos)):
        # if clickable_infos[i]['text'] != "" and clickable_infos[i]['text'] != "icon: None":
    ...
    #无法实现滑动操作prompt += "Swipe (x1, y1), (x2, y2): Swipe from position (x1, y1) to position (x2, y2).\n"
    if keyboard:
    ...
    #prompt += "Stop: If you can judge whether this app complies the rule, you can choose this action to terminate the operation process.\n"
    # prompt +="Back: Back to the previous page.\n"
    prompt += "\n\n"
...
def get_evaluate_prompt(i,clickable_infos1, clickable_infos2, width, height, keyboard1, keyboard2, summary, action, add_info='',free_flag=False):
    ...
    #prompt += f"The rules is: {instruction1+instruction2}. In the process of completing the requirements of instruction, an operation is performed on the phone. Below are the details of this operation:\n"
...
#     return prompt
#
# def get_process_prompt(i,thought_history, summary_history, action_history, completed_content, add_info):
#     instruction=get_instruction(i,1)
#     prompt = "### Background ###\n"
#     prompt += f"There is a task which is: {instruction}. You are a helpful app compliance inspector and are helping me operate the Android auto app to judge whether this app complies the rules.\n\n"
#
#     if add_info != "":
#         prompt += "### Hint ###\n"
#         prompt += "There are hints to help you complete the user\'s instructions. The hints are as follow:\n"
...
#         for n in range(len(summary_history)):
#             operation = summary_history[n].split(" to ")[0].strip()
#             prompt += f"Step-{n+1}: [Operation thought: " + operation + "; Operation action: " + action_history[n] + "]\n"
#         prompt += "\n"
#
